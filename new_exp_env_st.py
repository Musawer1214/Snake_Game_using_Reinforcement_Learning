...
class SnakeGameEnv:
    """
    Environment with:
      - Fixed obstacles across all episodes
      - Foods re-spawn each episode and when eaten
      - Local vision grid + frame stacking for a CNN-based DQN
    """
    def __init__(self):
        ...
        # ----------------------------------------------------------------
        # 1) Create Obstacles Once
        # ----------------------------------------------------------------
        self._place_obstacles_once()

        # Then call reset() to place snake and foods
        self.reset()

    def _place_obstacles_once(self):
        """Generate obstacles only once; these remain fixed throughout training."""
        for _ in range(NUM_OBSTACLES):
            while True:
                x = random.randint(0, (WIDTH - BLOCK_SIZE) // BLOCK_SIZE) * BLOCK_SIZE
                y = random.randint(0, (HEIGHT - BLOCK_SIZE) // BLOCK_SIZE) * BLOCK_SIZE
                # Just ensure we don't put obstacles on top of each other
                if (x, y) not in self.obstacles:
                    self.obstacles.add((x, y))
                    break

    def reset(self):
        """Reset the episode: snake to center, fresh foods, same obstacles."""
        self.direction = 0
        self.score = 0
        self.game_over = False

        # Center of screen for snake
        mid_x = WIDTH // 2
        mid_y = HEIGHT // 2

        # Initial snake: 3 blocks horizontally
        self.snake = [
            (mid_x, mid_y),
        ...
        # Clear old food items, re-place new ones each episode
        self.food_items = []
        self._place_foods()

        # Clear and re-init the grid stack
    ...
    def _place_foods(self):
        """Place multiple red & blue foods randomly (each episode)."""
    ...
    def _spawn_food(self, reward, color):
        """
        Spawn a single food in an empty location (not on snake or obstacle).
        The obstacles are fixed, so we skip those.
        """
        while True:
            x = random.randint(0, (WIDTH - BLOCK_SIZE) // BLOCK_SIZE) * BLOCK_SIZE
            y = random.randint(0, (HEIGHT - BLOCK_SIZE) // BLOCK_SIZE) * BLOCK_SIZE
            # Must not be on snake or an obstacle
    ...
    def step(self, action):
        ...
        # Out of bounds or snake-body collision => game over
        ...
        if eaten_idx >= 0:
            # Respawn that food somewhere else
    ...
    def _random_new_food(self, color, reward):
        """
        Respawn an existing food in a new empty location.
        """
        while True:
            x = random.randint(0, (WIDTH - BLOCK_SIZE) // BLOCK_SIZE) * BLOCK_SIZE
            y = random.randint(0, (HEIGHT - BLOCK_SIZE) // BLOCK_SIZE) * BLOCK_SIZE
    ...
    def _move(self, action):
        """
        Update direction and move head one block.
...
def train_snake(num_episodes=100):
    ...
    pygame.display.set_caption("Snake RL - Fixed Obstacles, Continuous Food")
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            # Draw obstacles (fixed across episodes)
...
if __name__ == "__main__":
    final_scores = train_snake(num_episodes=100)
